...
//   ierr = PetscPrintf(PETSC_COMM_WORLD,"tol = %f\n", tol);CHKERRQ(ierr);
  ...
  */
  ...
  for (i=rstart; i<rend; i++) 
  {
    ...
    ierr   = VecSetValue(f,i,sin(l*pi*(left + i*dx)),INSERT_VALUES);CHKERRQ(ierr);
  }
...
//   ierr = MatView(A,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
  ...
  */
  ...
  ierr = VecScale(f,bt);CHKERRQ(ierr);
...
//   ierr = PetscPrintf(PETSC_COMM_WORLD,"A*C = %f\n", at*ct);CHKERRQ(ierr);
  ...
  */
  ...
  while(PetscAbsReal(norm-normt)>tol){
     ...
     if(rank == 0){
      ...
      ierr = VecAssemblyEnd(b);CHKERRQ(ierr);
      ierr = VecAXPY(b,1,f);CHKERRQ(ierr);
      ierr = PetscPrintf(PETSC_COMM_WORLD,"step = %d, norm = %f\n",step,norm);CHKERRQ(ierr);
   }
