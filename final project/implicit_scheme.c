...
{
  Vec            x, b, u, ut, u0;          /* approx solution, RHS, exact solution */
  ...
  PetscScalar    zero = 0.0,value[3],omega = 1.6,dx = 0.01, kappa = 1.0, dt=0.01, rho=1, c=1, h=0;
  PetscScalar    at = kappa*dt/(rho*c*dx), bt = dt/(rho*c), ct = 2*dx*h/kappa;
...
//   ierr = PetscPrintf(PETSC_COMM_WORLD,"nlocal = %D\n",nlocal);CHKERRQ(ierr);
  ...
  */
//   ierr = PetscPrintf(PETSC_COMM_WORLD,"--------------->rstart = %d,  -------> rend = %d\n",rstart,rend);CHKERRQ(ierr);
  if (!rstart) 
  {
    rstart = 1;
  ...
  */
  ...
  while(PetscAbsReal(norm-normt)>tol){
     normt= norm;
     ierr = KSPSolve(ksp,z,x);CHKERRQ(ierr);
...
// //   }
  ...
  */
  ...
  ierr = VecDestroy(&z);CHKERRQ(ierr); ierr = MatDestroy(&A);CHKERRQ(ierr);
